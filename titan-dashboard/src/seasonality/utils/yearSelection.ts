...
/** Unique calendar years present in OHLC, newest first. */
...
/** Drop bars whose calendar year is in `excludedYears`. Empty → no filter. */
...
/** Toggle year in exclusion list (Seasonax-style: checked = included). */
export function toggleExcludedYear(excluded: readonly number[], year: number): number[] {
