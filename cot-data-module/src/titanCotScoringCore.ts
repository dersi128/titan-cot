...
export type OpenInterestSnapshot = {
  /** Week-over-week change in open interest (contracts), when available. */
  weeklyChange?: number;
  /** Optional 26-week percentile of OI level (0–100). */
  index26w?: number;
};
...
export type TitanCotScoringResult = {
  ...
  verdict: CotVerdict;
  marketPhase: string;
  /** Component breakdown for narratives (optional diagnostics). */
  components: {
...
/** Weeks commercials have stayed in the same 26W extreme (bull >80 or bear <20). */
...
export function buildPlainEnglishExplanation(input: {
  ...
  parts.push(
    `TITAN COT score ${result.score} → ${result.verdict}. ${BIAS_DISCLAIMER}`,
  );
...
/** Map legacy verdict strings from older deployments (optional UI compat). */
